...
case class Diagrammer(c: Component) {
  ...
  def draw(c: Component, w: Writer): Unit = {
    ...
    for (x <- c.getAllIo) {
      ...
      w.write(
        s"""  { rank=$rank; sig_${x.hashCode()} [label="${x.getName()}" style="$style" fillcolor="$color" shape=$shape] }\n""")
    }

    def writeTargets(e: Expression): Unit = {
      val style = e match {
        case bt: BaseType if bt.isReg => "bold"
        case _ => ""
      }
      e match {
        ...
        case l: Literal => w.write(s""" {sig_${l.hashCode()} [label="${l.getValue()}"]}\n""")
        case uop: UnaryOperator =>
          w.write(s"""  {sig_${uop.hashCode()} [label="${uop.opName}" style="$style"]}\n""")
          w.write(s"""  sig_${uop.source.hashCode()} -> sig_${uop.hashCode()}\n""")
        case bop: BinaryOperator =>
          w.write(s"""  {sig_${bop.hashCode()} [label="${bop.opName}" style="$style"]}\n""")
          w.write(s"""  sig_${bop.left.hashCode()} -> sig_${bop.hashCode()}\n""")
          w.write(s"""  sig_${bop.right.hashCode()} -> sig_${bop.hashCode()}\n""")
        case cop: ConstantOperator => ???

        case c: Cast =>
          w.write(s"""  {sig_${c.hashCode()} [label="${c.opName}" style="$style"]}\n""")
          w.write(s"""  sig_${c.input.hashCode()} -> sig_${c.hashCode()}\n""")

        case a: BitVectorBitAccessFixed => // stop decent here?
          w.write(s"""  {sig_${a.hashCode()} [label="x[${a.bitId}]" style="$style"]}\n""")
          w.write(s"""  sig_${a.source.hashCode()} -> sig_${a.hashCode()}\n""")
        case a: BitVectorRangedAccessFixed =>
          w.write(s"""  {sig_${a.hashCode()} [label="x[${a.hi}:${a.lo}]" style="$style"]}\n""")
          w.write(s"""  sig_${a.source.hashCode()} -> sig_${a.hashCode()}\n""")
    ...
    c.dslBody.walkStatements {
      case d: BaseType with DeclarationStatement =>
        ...
        w.write(s"""  {sig_${d.hashCode()} [label="${d.getName()}" style="$style"]}\n""")
      case da: DataAssignmentStatement => da.walkDrivingExpressions(writeTargets)
        w.write(s"""  sig_${da.source.hashCode()} -> sig_${da.target.hashCode()}""")

      case ia: InitAssignmentStatement =>
      case s: SwitchStatement =>
        w.write(s"""  {sig_${s.hashCode()} [label="switch" style="red"]}\n""")
