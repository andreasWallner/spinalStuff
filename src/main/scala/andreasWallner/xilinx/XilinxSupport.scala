...
object Helper {
  def engineeringNotation(d: BigDecimal, unit: String, decimalDigits: Int = 3): String = {
    Seq(
      ...
      case Some((prefix: String, multiplier: BigDecimal)) =>
        (d / multiplier)
          ...
          case v if v.isWhole() => v.toInt.toString() + (if (prefix != "") prefix else unit)
          case v => // "thankfully" BigDecimal.toString is not locale aware...
            v.toString().replace(".", if (prefix != "") prefix else unit).replaceAll("0*$", "")
        }
      case _ => throw new NumberFormatException(f"No valid SI-prefix available for $d")
    }
  }
}
