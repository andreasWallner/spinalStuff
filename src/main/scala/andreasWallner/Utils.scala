...
object Utils {
  ...
  class DumpAST(indent: String) {
    ...
    def printAttributes(d: Data): Unit = {
    ...
    def printAttributes(bt: BaseType): Unit = {
      printAttributes(bt.asInstanceOf[Data])
      print(if (bt.isReg) s"${bt.clockDomain} " else "")
    }

    def dump(e: Expression): Unit = {
      print("E " + indent + e.getClass.toString + " " + e.toString + " (")
      e match {
        case bt: BaseType => printAttributes(bt)
        case d: Data => printAttributes(d)
        case _ =>
      }
      println(")")
    ...
    def dump(s: Statement): Unit = {
      print("S " + indent + s.getClass.toString + " " + s.toString + " (")
      s match {
        case bt: BaseType => printAttributes(bt)
        case d: Data => printAttributes(d)
        case _ =>
