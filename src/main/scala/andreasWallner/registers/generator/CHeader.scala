...
class CHeader(intf: BusComponent, offset: Long = 0, comments: Option[Seq[String]]=None) {
  ...
  def overrideName(newName: String): CHeader = {
    name = newName
    this
  }
  ...
  private def writeRegisterOffsets(intf: BusComponent, writer: Writer): Unit = {
    intf.elements.foreach {
      case reg: Register =>
        writer.write(f"#define ${name.toUpperCase()}_${reg.name.toUpperCase()}_OFFSET 0x${reg.address + offset}%04x\n")
    } // TODO use correct literal macro/postfix
    writer.write("\n")
  }

  private def writeBankStruct(intf: BusComponent, writer: Writer): Unit = {
  ...
  private def writeRegisterStructs(intf: BusComponent, writer: Writer): Unit = intf.elements.foreach {
    case r: Register => writeRegisterStruct(r, intf.dataWidth, writer)
    case _ =>
  }

  private def writeRegisterStruct(reg: Register, dataWidth: Long, writer: Writer): Unit = {
  ...
  private def writeFieldInfos(intf: BusComponent, writer: Writer): Unit = {
