...
class KvasirHeader(
  ...
  def overrideName(newName: String): KvasirHeader = {
    name = newName
    this
  }
  ...
  private def accessTypeStr(field: Field): String =
  ...
  private def writeOneIgnore(accessType: AccessType): Boolean =
    accessType match {
  ...
  private def writeZeroIgnore(accessType: AccessType): Boolean =
    accessType match {
      case AccessType.RO => true
      case AccessType.RW => false
      case AccessType.RC => true
  ...
  private def usedMask(register: Register): Long = register.fields.map(f => if(f.accessType != AccessType.NA) f.section.bitmask else 0).reduce((a, b) => a | b)
  private def unusedMask(register: Register): Long = Section(component.dataWidth.toInt - 1, 0).bitmask ^ usedMask(register)
  private def ignoreMask(register: Register, pred: AccessType => Boolean): Long =
